...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    Hh = 1 + (input_dim[1] + 2*p - filter_size)
    Ww = 1 + (input_dim[2] + 2*p - filter_size)
    width_pool = 2
    height_pool = 2
    stride_pool = 2
    Hp = (Hh - height_pool) / stride_pool + 1
    Wp = (Ww - width_pool) / stride_pool + 1
  ...
  def loss(self, X, y=None):
    ...
    l1 = np.sum(W1 * W1)  + np.sum(W2 * W2) + np.sum(W3 * W3)
    loss += 0.5 * self.reg * l1
